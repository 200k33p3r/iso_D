      program lowmass
c     this subroutine reads in an exsiting isochrone file, and outputs
c     a file suitable for making a fake CMD in F606W and F606W-F814W.
c     it adds a bunch of points to the lower main sequence  to ensure smooth
c     operation  of the fake cmd program 

      implicit none
      integer maxiso,maxpt
      real*8 dmass
      parameter (maxiso = 90)     !maximum number of imput isochrone ages
      parameter (maxpt = 800)   !maximum points on a given isochrone
      parameter (dmass = 0.002) !mass incresese between lower MS iso points
      character*80 hd1
      character*128 hd2
      character*1 cext
      integer npts(maxiso),niso,numages,ntrk,nextra,totalextra
      real*8 minmass
      real*8 isoage(maxiso),mixlen,ovsh,yy,zz,zeff,feh,afe
      real*8 mass(maxpt),logg(maxpt),logteff(maxpt),logl(maxpt),
     $     num0(maxpt),num235(maxpt),num400(maxpt),vv(maxpt),
     $     vi(maxpt),f606(maxpt),f606f814(maxpt),f6new,f68new
      real*8 massstep ,tmass
      integer nhit(maxiso),nextrapts(maxiso)
      integer i,j,k,eep,itable,kk,n_args
      character*10 cniso,cminmass


!     get number  number of isochrone ages and
!     the minimum mass which should NOT be replaced by the low mass tracks
      n_args = command_argument_count()
      if (n_args /=2) then
         write(*,*)'./prepiso num_iso_ages, minmass'
         stop
      end if
      call get_command_argument(1,cniso)
      cniso = trim(cniso)
      read(cniso,*) niso
      call get_command_argument(2,cminmass)
...
c  read in the isochrones and add points

c      open(unit=11,status='old')
      open(unit=12)

      do i=1,niso
         read(11,200)hd1
 200     format(a80)
         read(11,210)npts(i),mixlen,ovsh,isoage(i),yy,zz,zeff,feh,afe
 ...
 250        format(I3,A1,F9.6,2F10.6,F11.7,3E16.9,8F7.3)
            if(mass(j).lt.minmass) nhit(i)=j
         enddo
c         write(*,*)'i,nhit(i):', i,nhit(i)
         nhit(i) = nhit(i) + 1
c         write(*,*)'age,minmass:',isoage(i),mass(nhit(i))
         read(11,*)
         read(11,*)

!     need to caclulate how many extra points we are going to add
         nextrapts(i)  = 0 
         do j = 2, nhit(i)
            nextra = nint ((mass (j) - mass(j-1) ) /dmass )
!            write(*,*)j, mass(j), (mass (j) - mass(j-1) ) /dmass,nextra
            nextrapts(i) = nextrapts(i) + nextra
         enddo
!         write(*,*)'extra points added: ', isoage(i),nextrapts(i)
c write out header info
         write(12,200)hd1
         write(12,220)npts(i) + nextrapts(i),
     $        mixlen,ovsh,isoage(i),yy,zz,zeff,feh,afe
 220     format('#',I3,F9.6,F8.4,F7.0,F8.4,2E12.4,2F6.2)

         write(12,299)
 299     format('#EEP   MASS    F606W    F606W-F814W')
         kk = 0
!     calculate extra points
         do j = 1,nhit(i)-3
            kk = kk + 1
            write(12,300) kk, mass(j), f606(j),f606f814(j)
 300        format(I3,2x,F9.6, 2F7.3)
            nextra = nint ((mass (j+1) - mass(j) ) /dmass )
            tmass = mass(j)
            massstep = (mass (j+1) - mass(j) )/(nextra + 1 )
!            write(*,*)'age,masstep =', isoage(i),massstep
            do k = 1,nextra
               tmass = tmass + massstep
!     write(*,*)'tmass:',tmasss
!     do the interpolation
               call parrot(mass,f606,j+1,npts(i),tmass,f6new,1)
               call parrot(mass,f606f814,j+1,npts(i),tmass,f68new,1)
               kk = kk + 1
               write(12,300) kk,tmass,f6new,f68new
            enddo
         enddo
         
!     last 2 mass points just do a linear interpolation
         do j = nhit(i) - 2, nhit(i) -1
            kk = kk + 1
            write(12,300) kk, mass(j), f606(j),f606f814(j)
            nextra = nint ((mass (j+1) - mass(j) ) /dmass )
            tmass = mass(j)
            massstep = (mass (j+1) - mass(j) )/(nextra + 1 )
!     write(*,*)'age,masstep =', isoage(i),massstep
!           write(*,*) 'now doing linear interpolation'
!           write(*,*)'mass:',mass(j),mass(j+1)
            do k = 1,nextra
               tmass = tmass + massstep
!     write(*,*)'tmass:',tmasss
!     do the interpolation
               call linint(tmass,mass(j),mass(j+1),f606(j),f606(j+1),
     $              f6new)
               call linint(tmass,mass(j),mass(j+1),f606f814(j),
     $              f606f814(j+1), f68new )
               kk = kk + 1
               write(12,300) kk,tmass,f6new,f68new
            enddo
...
c **** MC changed format
 55      FORMAT(4X,F4.2,3x,E10.4,3x,E10.4,18x,F5.2,6x,F6.4)
